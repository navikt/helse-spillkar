...
internal class SpillkarKlient(
    private val httpClient: HttpClient,
    private val azureTokenProvider: AzureTokenProvider,
    env: Map<String, String>
) {
    private val cluster = env["NAIS_CLUSTER_NAME"]?.lowercase() ?: "prod-gcp"
    private val scope = "api://$cluster.tbd.spillkar-api/.default"

    fun inngangsvilkårVurdert(packet: JsonMessage) {
        post(
            endepunkt = "inngangsvilkår-vurdert",
    ...
    fun inngangsvilkårsvurdering(packet: JsonMessage): ArrayNode {
        val responseJson = post(
            endepunkt = "inngangsvilkårsvurdering",
            requestBody = packet.toJson(),
            callId = UUID.fromString(packet["@id"].asText()),
            forventetResponseCode = 200
        )
        return responseJson.path("inngangsvilkårsvurdering") as ArrayNode
    }
    ...
    private fun post(endepunkt: String, requestBody: String, callId: UUID, forventetResponseCode: Int): JsonNode {
        ...
        val request = HttpRequest
            ...
            .uri(URI("http://spillkar-api/$endepunkt"))
        ...
        check(response.statusCode() == forventetResponseCode) {
            "Feil fra Spillkar-api. Forventet HTTP $forventetResponseCode, men fikk ${response.statusCode()}"
        }
